...
class CreativeControlLoader:
    ...
    def load_creative_control(self, run_id):
        rows = self._load_rows("Creative_Control")

        for row in rows:
            if str(row.get("run_id", "")).strip() == str(run_id).strip():
                return row

        raise ValueError(f"Creative control not found for run_id: {run_id}")
